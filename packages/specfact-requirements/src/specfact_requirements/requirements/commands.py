...
def _optional_source_path(ctx: typer.Context, source_name: str) -> Path | None:
    if source_name == "file" and ctx.args:
        raise typer.BadParameter("--from-file does not accept a positional source path")
    if len(ctx.args) > 1:
        raise typer.BadParameter("provide at most one source path")
    return Path(ctx.args[0]) if ctx.args else None


@app.command(
    "import",
    help="Import local, OpenSpec, or Spec Kit requirement evidence into a project bundle.",
...
def import_command(
    ...
    source_path = _optional_source_path(ctx, source_name)
    if source_name == "file":
