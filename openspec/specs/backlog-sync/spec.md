...
This specification defines the modules-owned backlog synchronization contract,
including issue/task sync behavior, external artifact ownership boundaries, and
managed `.specfact` state updates.
...
- **THEN** the command SHALL rewrite that managed artifact deterministically when its computed baseline content changes
