# Tasks: sync-01-unified-kernel

## 1. Branch and dependency guardrails

- [ ] 1.1 Create dedicated worktree branch `feature/sync-01-unified-kernel` from `dev` before implementation work: `scripts/worktree.sh create feature/sync-01-unified-kernel`.
...
- [ ] 4.3 Run `openspec validate sync-01-unified-kernel --strict` and resolve all issues.

## 5. Delivery

- [ ] 5.1 Update `openspec/CHANGE_ORDER.md` status/dependency notes if implementation sequencing changed.
- [ ] 5.2 Open a PR from `feature/sync-01-unified-kernel` to `dev` with spec/test/code/docs evidence.
