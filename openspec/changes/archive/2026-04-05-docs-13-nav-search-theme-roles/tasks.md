...
- [x] 8.1 Define a canonical source for per-module release history that is updated automatically whenever a module version is published
- [x] 8.2 Add homepage or overview rendering for recent module release history using that canonical source, with graceful fallback when no history is available
- [x] 8.3 Extend the publish flow so `.github/workflows/publish-modules.yml` writes the new release-history entry alongside the existing registry metadata update
- [x] 8.4 Verify the rendered changelog entries match the canonical release-history source for all official modules
