...
- [ ] 5.1 Prepare the proposed semver bump, manifest bindings for the existing preflight and new implementation-check workflow identities/digests, and matching `core_compatibility` lower bounds in both bundle manifest and registry entry set to the first released core containing the final #684 interfaces; prepare below-bound rejection plus exact-bound/supported-newer compatibility cases and publication inputs. Feature-branch artifacts are not publishable identities.
- [ ] 5.2 After 5.1, rerun the complete release-surface matrix against the actual proposed merge tree, including `verify-modules-signature --payload-from-filesystem --enforce-version-bump`, manifest/registry `core_compatibility` equality, below-bound rejection, exact-bound and supported-newer compatibility, bundle-import, contract, full test, independent analysis where applicable, and SpecFact code review; resolve every finding before merge.
- [ ] 5.3 Open, review, and merge the behavior-ready implementation PR to `dev` only after 5.2 passes, linking core #684, dogfood, assurance limits, metrics, and rollback evidence.
- [ ] 5.4 Allow only the canonical post-merge publication workflow to generate, sign, verify, compatibility-test, and propose the immutable #434 module, registry, checksum, signature, and history artifacts with matching manifest/registry `core_compatibility`; review and merge that publication PR only after every identity, lower-bound, compatibility, and rollback check passes.
