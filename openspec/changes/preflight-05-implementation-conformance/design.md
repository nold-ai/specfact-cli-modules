...
Worktree and index extraction reuses C14 scope, sandbox, and toolchain primitives and implements the released core matrix exactly. Every snapshot base is the seal-bound implementation-lineage origin repository/base commit/base tree. A worktree snapshot additionally binds a worktree-manifest digest and includes staged, unstaged, and untracked state. An index snapshot additionally binds the exact index tree ID; untracked paths are absent unless staged as additions. A range snapshot binds full head commit/tree, separately authorized current delivery-target commit/tree, and origin-to-head ancestry; untracked paths are not representable. Every manifest retains additions, deletions, both rename endpoints, before/after modes, symlink target identity, and byte-preserving path identity. Rename classification is bound to producer, toolchain, and policy identity. Index claims execute against the captured index capsule; a differing worktree cannot satisfy staged evidence.
