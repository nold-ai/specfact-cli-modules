...
## Marketplace and combined checks — 2026-09-13 Europe/Berlin

The combined implementation passed 1,924 full tests, 1,924 smart tests, 28 contract tests and normal signed-commit formatting/type/lint/YAML/import/version and changed-line review gates. Strict OpenSpec, Markdown and planned Requirements evidence passed. Manual review from an unconfigured installed CLI failed closed on macOS (`unsupported_controller_platform`); the canonical source-configured commit review ran the host development surface and passed changed-line enforcement, retaining legacy findings rather than claiming Linux acceptance.

Review 3997957677 additionally required explicit main-marketplace selection. After correcting the test setup, its regression failed on the missing clean installation environment. The workflow now invokes the fresh venv executable with `env -i` and `SPECFACT_MODULES_BRANCH=main`, eliminating inherited registry, development-source and credential overrides before strict pinned artifact verification.

## Passing hosted candidate matrix — 2026-09-13 Europe/Berlin

[Run 34725587532, attempt 2](https://github.com/nold-ai/specfact-cli-modules/actions/runs/34725587532) passed cp311/cp312/cp313 at signed head b69900b5 (merge checkout ac66ac07). All ten analyzers executed on cold/warm/alternate/defective/repository runs as UID 1001. Clean runs passed; deliberately failing pytest assertions failed with exit 1. All three namespace-denial checks returned the expected offline-install launcher identity and UNKNOWN/1. Fresh caches under umasks 022/077 and offline warm cache identities matched; private writes succeeded and sealed writes returned EROFS. Downloaded reports were independently revalidated and summarized, with report SHA-256 values and retained findings, in CUSTOMER_CI_EVIDENCE.json.

The repository slice reported missing packaging type resolution and 70.6% coverage against the existing 80% policy; these remain real repository findings, not an all-clean repository claim. The public baseline installed was signed module 0.49.77, while analyzer execution used the candidate source. This is candidate acceptance, not corrected public-release acceptance. Minimum-core compatibility also passed all three ABIs.

The source-configured manual `specfact code review run --enforcement changed --bug-hunt --json --out .specfact/code-review.json <changed Python paths>` passed once the worktree was frozen. A prior run correctly rejected concurrent evidence edits with worktree_snapshot_changed_during_analysis. The passing report retains 419 findings including one legacy blocking finding and no changed-line blockers. Normal pre-commit review already validated staged implementation changes before push; neither local host-development report is Linux capsule acceptance.

## Release review follow-up — 2026-09-13, Europe/Berlin

PR #469 revealed shallow installed-path failure, pytest snapshot/project-runtime import precedence drift, and an optimization-disabled measurement guard. Specs were extended before tests and production edits. The focused pre-fix run (`hatch run python -m pytest tests/unit/specfact_code_review/run/test_release_review_regressions.py tests/unit/test_capsule_refresh_lock.py -q`) produced **4 failed, 12 passed**: both shallow-path cases raised IndexError, the real generated observer imported the project dependency, and `python -O` accepted beartype 0.0.0. After repair, these tests and the customer pytest selection regressions passed.

The child regression executes the generated observer under `-I -S` using controlled analyzer/builtin/project/snapshot roots. It proves snapshot precedence for a colliding customer module while analyzer, builtin and pytest decoys retain sealed precedence. The optimized measurement test also retains a passing pinned-version control. The wheel success fixture now includes populated RECORD rows. Documentation records the implemented lifecycle, final PyYAML closure and clean-baseline recovery after partial refresh; artifact upload uses the existing repository-pinned action SHA.

### Late PR #470 configured-pythonpath regression

A real generated observer running pytest with `pythonpath = src` and `--import-mode=importlib` loaded the nested dependency decoy from initial conftest before the fix: **1 failed, 3 passed**, exit 4 with `configured path shadowed sealed dependency`. The capsule Observer now uses a try-first initial-conftest hook to restore analyzer/builtin precedence after pytest applies configured paths, before default conftest loading. Configured customer src roots retain their order ahead of project dependencies. The repaired regression covers both conftest import safety and ordinary src-layout test imports; existing isolated observer and customer-selection regressions also pass. The hook applies only inside a capsule.
