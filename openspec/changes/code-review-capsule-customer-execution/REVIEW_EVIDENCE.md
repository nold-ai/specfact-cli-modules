...
Comment 3998036209 identified a scoped-pytest regression after the earlier hosted success: the new local path planned a complete suite for every scope and suppressed static execution when pytest planning failed. The repair uses actual assurance scope (independent of enforcement), keeps narrow selections bounded, and retains static results when only pytest is unavailable. Cached snapshot mapping has a dedicated regression. All 416 runner regressions and 60 gate tests pass. The expanded hosted gate includes a source-only review beside an unrelated failing test. This thread awaits push and hosted scope evidence.

## Release PR #469 follow-up — 2026-09-13

The independent review agent classified 13 threads: eight actionable comments repaired in this follow-up and five descriptor-lifetime false positives. Runtime regressions cover snapshot/project import precedence, shallow installed provenance and optimized measurement guards; workflow, fixture and lifecycle/provenance corrections are included. Exact-head checks and review remain required before merging the fix.

Descriptor comments 3998971852, 3998971856, 3998971858 and 3998971860 refer to descriptors registered by `_open_payload_directory` on the enclosing ExitStack. Comment 3998971861 has an immediate `stack.callback(os.close, descriptor)` before reads. Actual-function probes recorded six descriptors for normal and exceptional composition-directory runs, nine for bootstrap installation and twelve for mount anchors; all were closed after each scope. This proves the flagged lifetime paths and does not claim complete Linux acceptance. Secure no-follow traversal is unchanged.

Late PR #470 comment 3999118492 identified pytest pythonpath reordering after observer startup. The follow-up adds a real conftest/test subprocess regression and restores trusted roots in the early initial-conftest hook. Exact-head quality and customer checks must be repeated after this payload change.
