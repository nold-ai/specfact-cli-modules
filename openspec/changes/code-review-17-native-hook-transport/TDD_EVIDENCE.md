...
## Reviewed native capabilities and actual Node dependency (2026-09-16)

The source candidate now explicitly declares supported git/uname/sed capabilities and pins the real BasedPyright dependency nodejs-wheel-binaries24.16.0. The transport permits only this reviewed metadata insertion, that dependency insertion and the existing two analyzer pin replacements. It still compares every resulting pyproject byte with the exact expected transformation. `NATIVE_TOOLS_DECLARATION_RED.txt` records the legitimate new declaration rejected before implementation, with the unrelated-mutation rejection control passing. After updating the bounded guard, all53 integrated transport/Semgrep/BasedPyright diagnostic tests passed in7.91seconds. The source allowlist adds only the reviewed native-tools/Node policy code, tests, docs and failing/passing evidence; supplied snapshots still cannot alter hooks or workflow controls.

`BASEDPYRIGHT_SNAPSHOT_MODE_RED.txt` records a real cached-index identity failure before the diagnostic created its index directory with mode `0700`, matching production's `TemporaryDirectory` root. The focused helper set then passed **13 tests in 0.61 seconds**. Production source identity rules are unchanged. This corrects a deterministically reproduced reconstruction difference; native run `35076571906` stopped on an offline cache miss before replay and retained no raw basedpyright output, so exclusive causality for that run is not asserted. Its artifact is `10439215453`, digest `sha256:8c98ecb7dbf0baa3119c3ddd1a8fd94364237ee270fc12e5345e249973a0af94`. No additional baseline dispatch is required or claimed; a future combined native run may opt into the corrected diagnostic.

Final combined transport validation after the root-mode correction:54 focused tests passed in8.19seconds. Fresh four-file bug-hunt `/private/tmp/specfact-478-transport-final-combined-review.json` is PASS_WITH_ADVISORY with five informational length/readability suggestions and zero warnings/errors. Retain the existing explicit receipt/replay and native environment fixture arrangements. The added test_replay_preserves_real_cached_snapshot_identity deliberately keeps both actual index materialization and the diagnostic replay in one ordered fixture, with independent source and plan identity assertions; compressing these steps would hide which root mode is authenticated. No check, coverage threshold, failed hook or failed replay is waived.


## Fixed control-owned payload, 2026-09-16

The final reviewed source patch exceeds the existing inline workflow input limit. Keep the exact legacy JSON form and add only `{base, tree, sha256, patch_source: "control-checkout"}`. The fixed gzip data is loaded beside the checked-out control helper, never from a caller path or URL. An atomic no-follow/nonblocking open plus a regular-file descriptor check rejects symlinks and FIFOs without hanging. Compressed and expanded reads are each limited to1000000bytes. The shared digest, immutable base, allowlisted paths, development metadata transformation and resulting-tree checks remain authoritative. Receipts retain transport mode, compressed digest/size, raw size and fixed relative path; existing outer GITHUB_SHA binds the control checkout.

`CONTROL_PAYLOAD_RED.txt` records5 real pre-implementation failures with9 rejection controls passing: a genuine large Git patch could not be attached, the legacy receipt lacked transport evidence, and file-form identity tests stopped at the unsupported request shape. After implementation,49 transport tests passed in9.91seconds, including14 added cases. Ruff formatting/checks passed, basedpyright reported0errors/0warnings, Pylint rated10.00/10, and strict OpenSpec validation passed. These are local transport checks; they do not claim that final native hooks or customer acceptance passed. No workflow input, hook control, source payload or release authority was changed by this extension.

The frozen delivery payload contains 47 files, 312,668 raw bytes and 63,321 deterministic gzip bytes. A real `prepare_snapshot` call against a clean checkout at `efe7303efb32ac31379906a38e3f960ec4e7a744` passed the existing path/mode/development-declaration guards and produced tree `d5b0c610d24b179a82b7acb5dac9c15aa1681dcc`. Raw SHA256 is `307d0dac496eb5fe998a7f498e1ab862ddcfa600735bb7cfaf1efa5df66579b8`; compressed SHA256 is `7f914e81e8f6c7cb0b1224f504f724278f249c3c27554153ab54c6dc22b2bf1a`. This proves transport application and identity binding only; successful unchanged native hooks remain required before any delivery commit.

Final fixed-file transport review (`/private/tmp/specfact-478-control-file-review.json`) passed with two informational structure suggestions and zero warnings/errors. Both retained tests expose distinct receipt identity/report assertions and genuine Hatch/stdout behavior; the existing explicit readability dispositions still apply. Type checking, YAML, import boundaries, all seven manifest payload/version checks and all28 contracts pass. The binary data file contains the exact reviewed source patch, not executable controls; the normal signed commit hooks remain unchanged.

The captured CONTROL_PAYLOAD_RED.txt output has trailing whitespace normalized only for repository diff hygiene; failure messages and outcomes are unchanged.


## ASCII fixed payload preserves normal hook compatibility, 2026-09-16

The first normal signed commit attempt failed before commit creation: the unchanged code-review hook uses a strict UTF-8 staged text diff, and the binary gzip introduced an undecodable byte. `CONTROL_PAYLOAD_HOOK_RED.txt` retains the actual failure. The transport now reads only the fixed `scripts/native_hook_snapshot.patch.gz.b64` file as canonical ASCII base64, with no trailing newline. It independently bounds encoded input to 1,333,336 bytes, decoded gzip to 1,000,000 bytes and expanded patch to 1,000,000 bytes. Strict decoding and re-encoding equality reject invalid characters, whitespace, non-ASCII bytes and noncanonical padding bits. Receipt digest and compressed size still describe the exact decoded gzip, preserving source identity.

`CONTROL_PAYLOAD_ASCII_RED.txt` records seven failures against the old fixed binary filename and one passing actual Git staged-text-diff control before changing production code. The passing control demonstrates that the new data format is UTF-8 safe; the failures do not independently isolate each decoder rejection. After implementation, all 56 transport tests passed in 10.13 seconds, including explicit encoded-size, decoded-size, alphabet, non-ASCII, padding-bit and newline rejection controls. Ruff, basedpyright (zero errors/warnings), Pylint (10.00/10), strict OpenSpec and diff checks passed. The source payload replacement, normal hook retry and native workflow are subsequent validation; no hook or delivery source was modified by this correction.

Final ASCII payload verification: the fixed file is84,428 canonical ASCII bytes (no trailing newline), SHA256 `ab4dd10a8f88a953f02ee10e7463e3ac1f1bc72bd675f91dd457ef9e294ce12c`. Strict decoding reproduces the same63,321 gzip bytes and312,668 raw patch bytes recorded above. Real clean-baseline preparation again produced the exact `d5b0c610d24b179a82b7acb5dac9c15aa1681dcc` tree. Fresh manual review `/private/tmp/specfact-478-control-ascii-review.json` passed with the same two informational test-structure suggestions and zero warnings/errors. Unchanged hooks will be retried normally; no hook code or encoding policy was modified.

ASCII and hook RED captures likewise normalize trailing whitespace only; diagnostics and outcomes are retained.
