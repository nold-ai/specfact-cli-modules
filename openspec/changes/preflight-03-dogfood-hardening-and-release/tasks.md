...
- [ ] 4.2 Update the package manifest, registry metadata, structured release-history entry, generated references, and signed payload as one release surface through repository tooling; make the publication pre-check require strict arbitrary equality (`===`) to the selected released core version that contains #682, reject empty, matrix-unproven, ordinary equality (`==`), local-alias, wildcard, and future-range values, and prove that release's immutable tag, full commit, and full tree before signing.
