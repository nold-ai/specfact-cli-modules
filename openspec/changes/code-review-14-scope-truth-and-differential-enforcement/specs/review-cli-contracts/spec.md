...
- **AND** ordinary worktree analysis rejects selected symlinks before analyzer execution, binds the pre-analysis raw selected regular-path states and immutable `HEAD` tree identity or the format-correct empty tree when `HEAD` is proven unborn, uses that same base tree for raw and Git changed-line evidence, verifies the same identity before and after changed-line projection, and returns required `UNKNOWN` / exit `1` on selected-byte, path-type, presence, or base-tree drift instead of analyzing a dereferenced target or comparing analyzer findings with a later worktree state
