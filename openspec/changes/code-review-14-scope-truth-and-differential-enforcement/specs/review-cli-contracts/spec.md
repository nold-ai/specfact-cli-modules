...
- **AND** ordinary worktree analysis binds the pre-analysis raw selected-path states and immutable `HEAD` tree identity, verifies the same identity before and after changed-line projection, and returns required `UNKNOWN` / exit `1` on selected-byte, path-type, presence, or base-tree drift instead of comparing analyzer findings with a later worktree state
