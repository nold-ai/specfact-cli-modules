...
- **AND** each governed `.pyi` is absent from Coverage and CrossHair reconciliation but remains exactly once in Ruff, Radon, each activated Semgrep pass, ai-bloat-ast, ast-clean-code, basedpyright, and Pylint eligible/invoked manifests under signed `analyzer-input-kinds-v1`
...
- **GIVEN** an applicable side contains governed standalone `.pyi` stubs and no tracked regular governed production `.py`
- **WHEN** analyzer applicability is derived from signed `analyzer-input-kinds-v1`
- **THEN** `contracts` records `execution_state=not_applicable`, `evidence_outcome=NOT_APPLICABLE`, empty eligible/invoked manifests, the input-kind catalog digest, and `absence_reason=no_crosshair_executable_input`
- **AND** CrossHair is not launched and no `.pyi` path enters its argv
- **AND** Ruff, Radon, each activated Semgrep pass, ai-bloat-ast, ast-clean-code, basedpyright, and Pylint still include each stub exactly once
