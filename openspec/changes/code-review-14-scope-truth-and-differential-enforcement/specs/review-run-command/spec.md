...
- **THEN** comment-token manifests are compared before analyzer fingerprints under the signed `suppression-directives-v1` registry
- **AND** the new occurrence is an open blocking `introduced_inline_suppression` finding with normalized directive and immutable path/line/token evidence
- **AND** without a trusted external waiver, the missing base analyzer fingerprint is `unknown`, never `fixed`
- **AND** an exact trusted external waiver retains the base finding as open with waiver and `suppressed_at_head=true` evidence rather than calling it fixed; candidate source text, candidate policy, or inline justification cannot waive it
...
- **AND** installed distributions equal exactly the selected environment lock plus the separately named bootstrap allowlist
- **AND** unsupported environments, missing/extra packages, marker ambiguity, or edge/version/payload/closure drift yield UNKNOWN before analysis.
...
- **GIVEN** the authorized Pylint policy contains any canonical alias of non-empty `init-hook` or names a plugin outside the signed profile plugin manifest
- **WHEN** `pr-range-v1` prepares both analyzer sides
- **THEN** the member is UNKNOWN before Pylint launch and no hook/plugin code is imported
