...
- **THEN** it collects every exact selector from each changed/additional collectable test file and executes that set
- **AND** a failing changed-test assertion yields FAIL even when static analyzers are clean
- **AND** deleted tests or changed test-support inputs without deterministic exact head selectors yield UNKNOWN
