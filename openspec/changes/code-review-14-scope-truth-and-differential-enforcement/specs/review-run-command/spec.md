...
- **THEN** a fresh acquisition uses only the signed HTTPS registry/repository@manifest-digest locator, verifies config and ordered layer digest/diff-ID/media-type/size records plus the complete in-image wheel filename/platform/size/SHA-256 manifest before atomic cache publication, and never resolves a tag, caller URL, package index, or second artifact source
...
- **THEN** the projection binds the original controls, sets `include` to the sorted exact eligible snapshot-relative file manifest, and canonically clears `exclude` and `ignore` while retaining other authorized diagnostic settings
...
- **AND** an include mismatch, non-empty effective exclude/ignore, newly discovered whole-file control, or manifest mismatch yields UNKNOWN.
