...
- **AND** `module-code-payload-v1` binds package/version/checksum/signature, protected key fingerprint, loader/origin/root/source-release identities, deterministic full-payload verification result, every canonical payload member and content digest, allowed entry modules/modes, sealed destination, payload-manifest digest, interpreter/bootstrap identity, and final-root digest
