...
- **AND** an otherwise untouched directive whose complete physical line has a uniquely optimal exact-line correspondence after unrelated line insertion remains the same occurrence; correspondence is established across all same-path/family/token occurrences before any deterministic ordinal fallback, so inserting one repeated directive does not reclassify later proven occurrences; a unique physical directive line with no uniquely optimal correspondence is relocated and introduced, while non-unique, invalid, resource-unavailable, or aggregate-work-bounded correspondence is `UNKNOWN` rather than guessed
