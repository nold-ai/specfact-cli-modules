...
- **AND** an otherwise untouched directive whose complete physical line has a uniquely optimal exact-line correspondence after unrelated line insertion remains the same occurrence; correspondence is established in deterministic physical-line order across all same-path/family/token occurrences before any deterministic ordinal fallback, so inserting one repeated directive does not reclassify later proven occurrences; when aggregate work is exhausted, exact-line candidates not yet proven remain `UNKNOWN` rather than shifting ordinal partners; a unique physical directive line with no uniquely optimal correspondence is relocated and introduced, while non-unique, invalid, resource-unavailable, or aggregate-work-bounded correspondence is `UNKNOWN` rather than guessed
...
- **AND** the only accepted core identity is lightweight tag `v0.55.1`, commit `b1e517e60e669eaba15a18ecfa83ef5a9df65276`, tree `47984be5434d7ae65ed6908bf525a32053290337`, and exact installed version `0.55.1`; branch heads, ordinary `==0.55.1`, local versions, alternate commits/trees, and later unpublished core revisions are rejected
...
- **WHEN** the mandatory Radon member analyzes an A-ranked governed file at merge base and head
- **THEN** candidate and ambient Radon discovery is disabled, the effective signed options retain CC A..F and MI A..C with empty exclude/ignore and private output, and the governed file appears exactly once in the eligible and invoked manifests
- **AND** a discovered source, narrowed effective range, skipped path, outside output, unclassified option, or pinned-loader/catalog drift yields UNKNOWN rather than fixed or PASS.
...
- **THEN** the projection binds the original controls, sets `include` to the sorted exact eligible snapshot-relative file manifest, and canonically clears `exclude` and `ignore` while retaining the exact shared global `typeCheckingMode` and diagnostic settings
- **AND** initial `pr-range-v1` rejects non-empty target path-scoped `strict` and non-empty target `executionEnvironments` before launch; `strict=[]` may be emitted only after proving the target array absent/canonically empty
...
#### Scenario: basedpyright path-scoped strictness cannot be weakened

- **GIVEN** authorized basedpyright policy has a non-empty path-scoped `strict` array that produces additional diagnostics for either an unchanged strict-listed path or code renamed outside its prior path semantics
- **WHEN** authoritative per-snapshot projects are prepared
- **THEN** initial `pr-range-v1` returns `basedpyright_strict_projection_unsupported` UNKNOWN before launch rather than clearing the target array
- **AND** global `typeCheckingMode` and diagnostic settings remain unchanged and cannot substitute for the path-scoped strict rule set
- **AND** `strict=[]` appears in a generated projection only when the original effective target array was absent or canonically empty
...
- **AND** each governed `.pyi` is absent from Coverage and CrossHair reconciliation but remains exactly once in Ruff, Radon, each activated Semgrep pass, ai-bloat-ast, ast-clean-code, basedpyright, Pylint, and `contracts.icontract-static-scan` eligible/invoked manifests under signed `analyzer-input-kinds-v1`
...
#### Scenario: Standalone stubs do not activate CrossHair

- **GIVEN** an applicable side contains governed standalone `.pyi` stubs, no tracked regular governed production `.py`, and signed `icontract-static-activation-v1` reproduces the existing icontract-usage predicate as true
- **WHEN** analyzer applicability is derived from signed `analyzer-input-kinds-v1`
- **THEN** `contracts.icontract-static-scan` binds its activation manifest/digest, runs over every eligible stub, and retains its findings and ran/PASS-or-FAIL-or-UNKNOWN evidence
- **AND** only `contracts.crosshair` records `execution_state=not_applicable`, `evidence_outcome=NOT_APPLICABLE`, empty eligible/invoked manifests, the input-kind catalog digest, and `absence_reason=no_crosshair_executable_input`
- **AND** the parent contracts member derives from both components and cannot be NOT_APPLICABLE on that activated stub-only side
- **AND** false static activation preserves the existing rule boundary as component NOT_APPLICABLE with `no_icontract_usage`, while activation uncertainty is UNKNOWN
- **AND** CrossHair is not launched and no `.pyi` path enters its argv
- **AND** Ruff, Radon, each activated Semgrep pass, ai-bloat-ast, ast-clean-code, basedpyright, Pylint, and `contracts.icontract-static-scan` still include each stub exactly once
- **AND** a mixed side runs CrossHair only over its eligible regular production `.py` set while retaining stubs solely in the stub-capable manifests
- **AND** input-kind/catalog/manifest drift is UNKNOWN rather than a synthetic CrossHair error or PASS.
...
- **THEN** `pytest-selection-controls-disabled-v1` parses aliases and values with the complete composed pytest 9.0.3, pytest-cov, and admitted-plugin option catalog before collection
...
- **AND** an unknown or unattested/unclassified plugin-added option or parser/help/catalog drift is UNKNOWN.
...
- **THEN** the complete composed pytest/pytest-cov/admitted-plugin `addini` catalog classifies `norecursedirs` as `selection_filter` and yields `pytest_selection_policy_unsupported` UNKNOWN before collection
- **AND** every other effective ini/TOML field is classified and bound; unknown or unattested/unclassified plugin-added fields and catalog drift are UNKNOWN
...
#### Scenario: A collection hook cannot hide one changed test in a collected file

- **GIVEN** an unchanged `conftest.py` hook would deselect a newly added failing test by name or marker while another selector from the same file remains visible and passes
- **WHEN** the sealed plugin/hook registry is built before complete-suite collection
- **THEN** `pytest-hook-dispositions-v1` identifies the repository hook by its canonical pytest hook spec and immutable snapshot origin and yields `pytest_collection_hook_unsupported` UNKNOWN before execution
- **AND** the path-level one-selector check cannot convert that selective omission to PASS
- **AND** any late registration, registry drift, or raw `pytest_itemcollected` node absent from the final inventory is independently UNKNOWN.

#### Scenario: Built-in item controls cannot hide a changed test in a collected file

- **GIVEN** a candidate adds a failing name-matching function or class with runtime `__test__ = False` beside a visible passing selector in the same test-candidate file
- **WHEN** pinned pytest 9.0.3 begins Python collection
- **THEN** the sealed observer records the candidate path, qualified name, object kind, exact name-match reason, and false `__test__` value at `pytest_collectstart` or before downstream `pytest_pycollect_makeitem` filtering
...
- **THEN** before calling `dir(TestCase)`, the sealed `pytest_collectstart` observer records a descriptor-safe raw MRO manifest of every name-prefix-eligible callable, validates `unittest-loader-controls-v1` prefix/pattern/sort and metaclass control identities, then records the exact `TestLoader.getTestCaseNames` inventory, inherited/resolved origin, and per-method `__test__`
...
- **WHEN** targeted pytest preflights and explicitly enables plugins with automatic loading disabled
- **THEN** the observed snapshot-free origin, parser catalog, and hook registry must exactly match `pytest-plugin-capabilities-v1`, and the composed catalog classifies the plugin's fields before target config is accepted
- **AND** the fixture-dependent suite collects under the identical plugin identity on both snapshots
- **AND** an unlisted, repository/user, identity-mismatched, dynamically registered, or collection-shaping plugin yields UNKNOWN; collection-shaping plugin semantics are outside initial `pr-range-v1`.
