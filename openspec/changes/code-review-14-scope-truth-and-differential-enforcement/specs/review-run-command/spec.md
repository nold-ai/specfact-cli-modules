...
Range enforcement SHALL analyze the resolved merge-base and head with identical shared analyzer/configuration identity: analyzer and toolchain identities; trusted target-policy/config source digest; projection algorithm/schema identity; and a canonical logical projection-map digest whose source-root placeholders are independent of either materialized path. Per-snapshot projected-config bytes and digests MAY differ because their absolute roots differ; those digests are materialization evidence and SHALL NOT be compared for raw equality. A mismatch in any shared identity field yields UNKNOWN, while a difference limited to the recorded side-specific root substitutions does not. The supplied base-ref tip SHALL NOT be used as the analyzer baseline when it differs from the merge base. Stable fingerprints SHALL classify findings as introduced, fixed, unchanged, or unknown. Before fingerprint comparison, the head file anchor for a resolved one-to-one rename SHALL be normalized to the recorded old/base path; copies and unpaired additions SHALL NOT be rename-normalized, and both original paths plus the rename fact SHALL remain in evidence. Before a missing base fingerprint can be classified fixed, the controller SHALL compare immutable base/head Python comment-token manifests under signed `suppression-directives-v1`. The registry SHALL cover the pinned profile's Ruff noqa forms; Pylint disable/disable-next/disable-all/legacy disable-msg/skip-file; basedpyright pyright/type-ignore, per-file report-disable, and `pyright: basic|standard` mode-lowering forms; Semgrep nosemgrep/nosem; Coverage no-cover/no-branch pragmas; and CrossHair off plus `per_condition_timeout=0` directives. Each occurrence binds registry/tool identity, normalized family/analyzer/rules, rename-normalized path, exact line, token digest, and occurrence-manifest digest. A new, changed, or relocated head occurrence is always open and blocking in CR14: diagnostic-suppression families emit `introduced_inline_suppression`, while basedpyright mode-lowering and CrossHair zero-timeout families emit `introduced_result_weakening_directive`; candidate source/policy/justification or an unauthenticated injected object cannot waive it. CR14 has no suppression-waiver ingestion or verification contract; authenticated exceptions are deferred to `governance-02-exception-management`. Any missing head fingerprint for the same analyzer/path is unknown rather than fixed. Tokenization/registry/normalization/comparison uncertainty is UNKNOWN. Changed-line intersection SHALL be evidence only and SHALL NOT be the sole introduction rule.
...
- **THEN** comment-token manifests are compared before analyzer fingerprints under the signed `suppression-directives-v1` registry, including Pylint `disable-all` and legacy `disable-msg=<rule-list>` aliases, basedpyright `pyright: basic|standard`, and CrossHair `crosshair: per_condition_timeout=0`
- **AND** the new occurrence is an open blocking class-specific finding—`introduced_inline_suppression` for diagnostic suppressions or `introduced_result_weakening_directive` for mode/timeout weakening—with normalized directive and immutable path/line/token evidence
...
#### Scenario: basedpyright referenced policy inputs are governed and sealed
...
- **AND** both sides receive projections from the same sealed logical graph, with baseline source identities rebound to their corresponding snapshot
- **AND** absolute, escaping, symlink, missing, cyclic, duplicate-canonical, unsupported, or over-bound reference input yields UNKNOWN.
...
- **THEN** the controller generates a distinct side project from `basedpyright-default-v1` logical JSON `{"include":["<SNAPSHOT_ROOT>"]}`
...
- **GIVEN** planned adapters require a Ruff extend bundle, basedpyright project and baseline/reference payload, Semgrep selected/fallback bundle, Pylint config, and pytest/coverage projections
...
- **THEN** the lock contains every evaluated direct and transitive distribution, marker/tag, dependency edge, exact version, wheel/RECORD payload identity, entry point, and canonical closure digest
- **AND** module pip dependencies constrain every locked distribution with exact `==`, including pluggy, nodejs-wheel-binaries, z3-solver, and all other transitive packages
- **AND** a separately provisioned analyzer-only environment contains exactly the selected environment lock plus the bootstrap allowlist, while unrelated controller/core/module host distributions remain outside its paths and are not counted as analyzer extras
- **AND** unsupported environments, missing/extra analyzer packages, host-path leakage, marker ambiguity, or edge/version/payload/closure drift yield UNKNOWN before analysis.
...
#### Scenario: Ruff analyzes every governed input without snapshot cache writes

- **GIVEN** authorized Ruff policy sets `cache-dir`, `force-exclude=true`, and an exclude pattern matching an eligible governed file
- **WHEN** Ruff runs for either immutable snapshot
- **THEN** the controller passes pinned `--no-cache` and `--no-force-exclude`, binds the effective flags and eligible/invoked manifests, and Ruff analyzes the explicit file
- **AND** no cache write targets the snapshot or read-only configuration roots
- **AND** a write, skipped file, ineffective flag, or option-catalog drift yields UNKNOWN.

#### Scenario: Pylint cannot ignore a governed input

- **GIVEN** authorized Pylint policy has `ignore`, `ignore-patterns`, or `ignore-paths` matching an eligible governed file
- **WHEN** the per-snapshot Pylint projection is built and invoked
- **THEN** all three whole-file controls are canonically empty while other authorized settings remain bound
- **AND** the eligible and exact argv input manifests match
- **AND** a non-empty effective ignore, unclassified whole-file control, or manifest mismatch yields UNKNOWN.

#### Scenario: basedpyright cannot ignore a governed input

- **GIVEN** authorized basedpyright policy has an `ignore` entry matching an eligible governed file
- **WHEN** the per-snapshot project artifact is built and invoked
- **THEN** the projection binds the original value and canonically clears `ignore` while retaining other authorized settings
- **AND** the eligible and exact argv input manifests match
- **AND** a non-empty effective ignore, newly discovered whole-file control, or manifest mismatch yields UNKNOWN.

#### Scenario: Semgrep reconciles every scanned target

- **GIVEN** a sealed Semgrep rule excludes an eligible governed file or an eligible file exceeds the tool's implicit/default target-size limit
- **WHEN** either Semgrep pass exits successfully
- **THEN** canonical JSON `paths.scanned` must equal the exact eligible explicit input manifest
- **AND** `paths.skipped` and its reasons remain evidence
- **AND** any missing, extra, excluded, oversized, unnormalized, or unreconciled path yields UNKNOWN rather than empty PASS.
...
- **THEN** the complete closed source/section set is manifested and the first applicable source is selected using the signed pinned-loader order
...
- **AND** native `[tool.pytest]` and legacy `[tool.pytest.ini_options]` are mutually exclusive in one selected pyproject
