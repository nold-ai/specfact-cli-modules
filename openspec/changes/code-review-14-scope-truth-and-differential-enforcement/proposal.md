...
- Materialize index analysis from staged blobs and range analysis from the resolved merge-base/head commit trees in isolated roots, bind them with content manifests, run every import-capable analyzer under a sanitized manifest-bound per-snapshot working directory/import environment, and reject mutation-capable options for snapshot scopes. Reject `focus=simplify` for range assurance because its finding filter is intentionally incomplete.
- Emit immutable scope evidence and fail closed as `UNKNOWN` when Git scope cannot be resolved.
- Reserve `assurance_kind=pr_range` for the complete governed merge-base-to-head Python selection: changed tests are always included, and path/facet/test/execution-level narrowing options are rejected before analysis. A complete unauthenticated local range remains available as `range_preview`; partial review remains available through explicitly labelled worktree or positional-file runs.
- Analyze the resolved merge-base and head source trees with the same pinned analyzer/config identities and classify findings as introduced, fixed, unchanged, or unknown using stable fingerprints; normalize a head file anchor through the resolved one-to-one rename map before comparison, and never use the supplied base-ref tip as the source-code differential baseline. Bind the resolved base-ref tip to an authenticated expected target commit/tree that the consumer independently verifies, freeze that current target-tip policy bundle, then pass safe per-snapshot config projections explicitly to both source snapshots so candidate head configuration cannot suppress its own findings, relative paths cannot escape into policy/worktree content, and already-promoted target policy cannot be ignored.
