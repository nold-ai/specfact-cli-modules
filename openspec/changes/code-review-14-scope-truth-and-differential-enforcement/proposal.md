...
- Emit immutable scope evidence and fail closed as `UNKNOWN` when Git scope cannot be resolved. Treat the closed `governed-policy-paths-v1` analyzer-policy/config set as governed range impact alongside Python; a candidate policy diff is never NOT_APPLICABLE and remains UNKNOWN until a separately trusted policy-promotion contract exists.
