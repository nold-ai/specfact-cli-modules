...
- Define the closed `pr-range-v1` analyzer profile and report its identity/coverage; incomplete required evidence is `UNKNOWN` when no validated blocker already proves FAIL. The released signed module SHALL provision the exact loader/analyzer versions bound by the profile and implementation checkpoint—never ambient bare or lower-bound resolution—including explicit `pytest==9.0.3` and `coverage==7.15.4` constraints.
