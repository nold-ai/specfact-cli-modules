...
- Reserve `assurance_kind=pr_range` for the complete governed merge-base-to-head Python selection: changed tests are always included, and path/facet/test/execution-level narrowing options are rejected before analysis. Partial review remains available through explicitly labelled worktree or positional-file runs.
