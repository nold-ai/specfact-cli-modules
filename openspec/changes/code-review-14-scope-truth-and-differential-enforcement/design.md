...
- The mandatory Semgrep clean-code pass receives an explicit `bundle_root` containing the authorized target-tip `.semgrep/clean_code.yaml` when present. When that file is absent, the resolver instead materializes the canonical `.semgrep/clean_code.yaml` payload from the exact installed, signed Code Review module artifact into a separate read-only fallback bundle and records the module/payload identities. The same selected clean-code payload is used for both source snapshots; Semgrep never searches an analyzed source tree, caller worktree, or process `.`. Missing, unsigned, version/checksum/signature-mismatched, or digest-mismatched fallback evidence yields `UNKNOWN`. This fallback applies only to the mandatory clean-code pass; `semgrep-bugs` remains conditional on governed bugs configuration in the authorized target-tip policy bundle.
...
Every index or range snapshot analyzer receives a `SnapshotInvocationContext` binding commit/tree/root, `cwd`, manifest-bound import roots, sealed non-editable toolchain/dependency environment, sanitized environment, process-private output/temp roots, sandbox backend/capability identity, and digest. Subprocesses run from the active snapshot root; inherited caller/worktree/policy-bundle import paths, `PYTHONPATH`, `PYTHONHOME`, editable-source roots, user site, and startup hooks are removed. Only that snapshot's declared import roots and the same sealed toolchain dependencies are added. Pytest plugin autoload is disabled and only pinned declared plugins are enabled. Every analyzer subprocess must consume this context; this explicitly includes Radon even though it does not import reviewed modules, plus Pylint init hooks, CrossHair, targeted pytest, and every other import-capable analyzer. Any caller-source import, undeclared plugin/root, or context mismatch is `UNKNOWN`.

Sanitized environment variables are not an isolation boundary. A dedicated `sandbox.py` capability layer SHALL launch every index/range analyzer in a fresh OS-enforced filesystem sandbox. CR14's only accepted initial backend is `linux-bwrap-v1`: an exact Bubblewrap executable selected through the sealed toolchain, with executable SHA-256, version, argv/profile schema, and capability-probe digest recorded. Protected CI SHALL provision that pinned backend; other operating systems, missing Bubblewrap, or a failed namespace/mount/network probe yield `UNKNOWN` until a separately accepted backend contract exists. The sandbox exposes the active snapshot read-only; only the exact projected config and sealed toolchain/dependency paths read-only; and one empty process-private output root plus private temp/cache/home roots writable. The caller checkout, opposite snapshot, claimed/trusted context, target-policy source and unselected fallback source, controller/report state, sibling analyzer outputs, host credentials, and network are absent or denied. It closes undeclared file descriptors and forbids symlink, mount, or path traversal across those boundaries. The trusted controller remains outside the sandbox, captures argv plus OS-observed exit/stdout/stderr, accepts artifacts only from that process's output root after exit, verifies declared artifact shape/digests plus source/config manifests, labels them `candidate_executed_observation`, then seals those exact bytes before starting a dependent comparison. Sealing proves immutability after collection, not that candidate code could not influence its own process or artifacts. Sandbox backend/version, mount/permission/network profile, capability probe, per-process root manifest/digest, and runtime-observation trust classification are evidence. Strict index/range assurance runs only on a backend that proves the complete isolation profile; unsupported or failed isolation is `UNKNOWN`, never an unsandboxed fallback.
...
The signed Code Review package SHALL include exactly `packages/specfact-code-review/src/specfact_code_review/resources/contracts/review-report-schema-1.6-consumer-matrix.json`. This checked-in machine-readable artifact binds schema 1.6's four authoritative statuses to valid legacy projections and strict/shadow exit behavior, and includes producer range_candidate/range_preview plus accepted/rejected trusted verification-envelope cases. It contains canonical minimal reports for PASS, FAIL, UNKNOWN, and NOT_APPLICABLE, required fields including `runtime_trust_model`, `adversarial_runtime_evidence`, and per-member observation trust, permitted `overall_verdict` values, expected `ci_exit_code`, a hostile-candidate-policy UNKNOWN case, and mismatch-invalid cases. Tests and the package manifest bind its digest.
...
The normative closed path allowlist is the one enumerated in `tasks.md`; no implementation path outside that list is permitted without first updating this OpenSpec change and adding a named failing test. Exactly six new source/test files are permitted:
