...
- [ ] 2.8 Add `test_targeted_pytest_coverage_classifies_failure_vs_unknown`, `test_targeted_pytest_rejects_head_skip_xfail_xpass_and_deselection`, and `test_targeted_pytest_new_input_absent_at_merge_base_is_not_applicable_for_that_side`; bind planned selector/JUnit/count/coverage digests, require exactly-once all-selected-passed head outcomes, classify head assertion/skip/xfail/xpass as FAIL, missing/duplicate/deselected/reconciliation/infrastructure/coverage gaps as UNKNOWN, cover a new production file plus test, and reject the baseline-absence exception when governed head production survives without tests.
...
- [ ] 3.12 Execute targeted pytest/coverage per snapshot with exact selector-to-JUnit reconciliation, outcome counts, absent-side, FAIL/UNKNOWN policy, environment identity, JUnit digest, and coverage artifact evidence.
