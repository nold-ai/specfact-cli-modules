...
- [ ] 1.15 Add `test_range_candidate_rejects_base_ref_mismatching_claimed_target_tip`, `test_producer_never_self_asserts_pr_range_from_context_file`, `test_trusted_consumer_envelope_upgrades_range_candidate_to_pr_range`, and `test_range_without_context_is_preview`; validate the outside-checkout regular context schema, bind provider/repository/PR-or-queue/ref/event/target/head/report digests, reject unsafe/mismatched context, prove the producer emits only candidate/preview, and require an independent workflow-native consumer envelope for effective pr_range.
...
- [ ] 3.24 Add and validate the closed consumer compatibility matrix package resource, including candidate/preview producer reports and trusted pr_range verification envelopes; bind its digest in package metadata/tests.
