...
- `packages/specfact-code-review/src/specfact_code_review/tools/semgrep_runner.py` and `tests/unit/specfact_code_review/tools/test_semgrep_runner.py` only to require the explicit target-policy `bundle_root` and consume the sealed executable/per-snapshot cwd/environment/output/context digest for both Semgrep passes, including explicit timeout/process/parse failure propagation; rule semantics remain unchanged.
