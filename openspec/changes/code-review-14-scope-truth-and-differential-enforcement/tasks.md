...
- [ ] 1.15 Add `test_range_candidate_rejects_base_ref_mismatching_claimed_target_tip`, `test_producer_never_self_asserts_pr_range_from_context_file`, `test_trusted_consumer_envelope_upgrades_range_candidate_to_pr_range`, `test_trusted_consumer_rejects_merge_base_mismatch`, `test_trusted_consumer_rejects_policy_config_identity_mismatch`, and `test_range_without_context_is_preview`; validate the outside-checkout regular context schema, bind provider/repository/PR-or-queue/ref/event/target/head/analyzed-merge-base/selected-policy-config/report identities and digests, reject unsafe/mismatched context, prove the producer emits only candidate/preview, and require an independent workflow-native consumer to derive and compare merge-base plus authorized policy/config before emitting the fully bound effective pr_range envelope.
...
- [ ] 3.12 Define the exact runner-owned `pr-range-v1` membership and serialize per-snapshot required/conditional coverage facts.
...
- [ ] 3.24 Add and validate the closed consumer compatibility matrix package resource, including candidate/preview producer reports, trusted pr_range verification envelopes, and merge-base/policy-config mismatch rejection cases; bind its digest in package metadata/tests.
...
- [ ] 4.7 After the signed consumer compatibility matrix passes and a separate core adoption change is accepted, give core the final merged commit/tree, module version, schema 1.6 contract and compatibility-matrix digest, archive/checksum/signature, signer, workflow, and auto-publish PR identities. The core PR SHALL keep `scripts/pre_commit_code_review.py` limited to `explicit_files` but make it consume authoritative schema 1.6 status/exit fields; a separate PR-range consumer SHALL reject any report or code path that cannot consume them. Core PR CI SHALL write the canonical context file under runner temp immediately before invocation, pass full base/head refs plus `--pr-context-file`, require producer `assurance_kind=range_candidate`, independently derive the expected merge base and authorized target-tip policy/config selection, compare the immutable report digest and all context/target/head/analyzed-merge-base/selected-policy-config identities to workflow-native trusted data, reject either mismatch as UNKNOWN, emit a separate verifier-identity-bound envelope containing every compared identity with `effective_assurance_kind=pr_range`, and migrate in shadow, warning, then enforce mode.
