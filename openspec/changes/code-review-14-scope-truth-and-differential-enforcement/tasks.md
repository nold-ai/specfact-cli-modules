...
- [ ] 2.14 Add `test_import_capable_analyzer_cannot_tamper_with_other_evidence_roots` in `tests/e2e/specfact_code_review/test_review_run_e2e.py`; run synthetic pytest and CrossHair targets that attempt absolute, relative, and symlink reads/writes across sentinel roots, prove denials and post-exit artifact sealing, and require UNKNOWN on escape or unexpected output.
...
- [ ] 2.18 Add `test_schema_1_6_consumer_compatibility_matrix_is_closed` in `tests/unit/specfact_code_review/run/test_findings.py`; validate the exact checked-in package resource, its four canonical reports, permitted legacy projections, strict/shadow exits, invalid mismatch cases, and canonical digest.
...
- [ ] 3.9 Route each index/range analyzer process through a fresh sandbox from `runner.py`; keep controller/context/other-side/sibling evidence outside it, accept only process-private outputs after exit, validate/seal them before reuse, and prohibit unsandboxed fallback.
