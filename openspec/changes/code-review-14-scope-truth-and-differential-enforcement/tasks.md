...
- [ ] 2.8 Add `test_targeted_pytest_coverage_classifies_failure_vs_unknown`, `test_targeted_pytest_rejects_head_skip_xfail_xpass_and_deselection`, `test_targeted_pytest_runs_changed_tests_in_test_only_range`, `test_targeted_pytest_reconciles_removed_baseline_selectors`, and `test_targeted_pytest_new_input_absent_at_merge_base_is_not_applicable_for_that_side`; bind selector/JUnit/count/coverage digests, require exactly-once all-selected-passed head outcomes, classify assertion/skip/xfail/xpass as FAIL and reconciliation/infrastructure gaps as UNKNOWN, collect/digest both selector inventories, normalize file renames, run every head selector, make removed baseline/deleted/non-expandable test inputs UNKNOWN, and cover a new production file plus test.
