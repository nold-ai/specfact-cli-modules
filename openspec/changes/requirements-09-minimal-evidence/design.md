...
4. A required selected acceptance case must be collected exactly once and pass. Failure/error/skip/missing/duplicate, XFAIL or XPASS is not proof; ordinary pass requires no expected-failure metadata, including empty markers. Ordinary full-suite skips remain separately visible under existing suite policy. Missing or malformed required output and wrong revision remain non-passing.
...
Exactly once is per canonical selector within the current selected candidate/environment/logical suite-or-shard/job-attempt unit, not globally across the matrix. Core derives the required unit set and current designated attempts from trusted CI metadata; each unit must satisfy its expected selector set. Do not cherry-pick older passing attempts when the current designated attempt is pending, failed, cancelled or unavailable. Ambiguous selection or duplicates inside one unit remain non-passing. Modules compares the shared eight-field execution-unit tuple `[candidate_binding, environment_binding, matrix_lane, suite_or_shard, provider, run_id, run_attempt, job_id]` using exact parsed-value equality alongside existing plan/source/environment bindings, retaining the first two binding values/types and requiring strings for the remaining six fields. Its compact JSON uses stable configured lane/suite IDs and actual provider run/attempt/job-execution IDs, not display names; non-matrix lanes use an empty string. Reuse existing CI metadata and outputs; no historical transcript, extra suite or new approval protocol is introduced.
