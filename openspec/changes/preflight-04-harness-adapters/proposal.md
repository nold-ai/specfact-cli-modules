...
- **NEW**: A later prerequisite-gated hatch3r integration that uses only a documented supported distribution/extension surface or a separately accepted upstream contribution; absent such a surface, no hatch3r adapter is shipped.
...
- Each external repository contribution requires its own accepted upstream issue/PR and must preserve that project's current contribution and packaging rules. The implementation must not write hatch3r's internal inventory or claim a third-party pack API that the selected release does not document.
...
- hatch3r support is additionally blocked until a selected release documents a supported third-party distribution/extension mechanism or a separately accepted upstream hatch3r change provides one.
