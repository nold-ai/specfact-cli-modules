...
- [ ] 1.14 Before any source edit, update `requirements-evidence.yaml` under the accepted mapping schema with the exact selectors from tasks 1.1–1.13, one stable opaque `expected_failure_id` per replayed selector, and the governed pre-R readiness-evidence touchpoint; verify each selector collects once and freeze the accepted mapping, plan, and readiness-evidence digests. Do not invent selectors on this planning-only branch.
