...
- Update **`openspec/config.yaml`** and **`openspec/CHANGE_ORDER.md`** so OpenSpec artifact rules reference the canonical rule system where appropriate; keep alias surfaces (`CLAUDE.md` if present) as thin pointers without duplicating long policy text.
...
- **Tests/tooling**: New or extended tests/validators for governance markdown frontmatter, required always-load files, and deterministic index semantics (under `tests/` or existing doc-validation harness as appropriate).
