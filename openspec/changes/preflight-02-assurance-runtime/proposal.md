...
- **NEW**: A future `specfact preflight run <change-id>` CLI that renders human and JSON results, supports read-only review by default, and separately persists the normalized validation result alongside the approved contract and seal while atomically advancing the canonical lineage-tip record only after explicit user approval.
