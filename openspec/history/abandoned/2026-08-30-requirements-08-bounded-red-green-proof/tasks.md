...
- [ ] 1.2 Add `test_final_reconciliation_keeps_current_execution_and_red_green_chronology_separate` and `test_report_schema_v4_discriminates_r08_from_r07_v3_and_legacy_v2`; assert mapping sidecars stay v2, finalized v2 retains `source_schema_version: 2` plus red-junit/digest-bound-ledger validation and is never reinterpreted, finalized v3 is R07 compatibility, v4 requires request plus both claims, missing v4 fields do not downgrade, and unsupported future versions are rejected.
...
- [ ] 1.13 Add `test_code_review_preserves_unrequested_not_evaluated_chronology`, `test_code_review_preserves_required_unavailable_or_untrusted_chronology_as_unknown`, `test_code_review_preserves_complete_trusted_contradiction_as_fail`, and `test_code_review_rejects_schema_v4_report_without_request_or_chronology_claim` in `tests/unit/specfact_code_review/run/test_commands.py`; require and retain the top-level Requirements gate decision, explicit request state, and canonical chronology claim object in schema-v4 input while routing v2/v3 only by their explicit versions, and assert Requirements provenance does not calculate review findings, score, verdict, or exit code.
...
- [ ] 2.3 Emit finalized report schema v4 while preserving explicit finalized-report v2 legacy and v3 R07 compatibility readers; never infer version from field absence. Delegate independent chronology reconciliation plus the public capsule input and explicit versioned `chronology_request`/`--chronology-request not-requested|required` input from the existing Requirements lifecycle/command seams. Default only backward-compatible current-only calls to `not_requested`; reject contradictory `not_requested` plus capsule input before reconciliation.
- [ ] 2.4 Require and retain the schema-v4 chronology claim object in Code Review context with distinct not_evaluated/unknown/fail/pass states and without verdict fusion; route finalized report v2 through legacy compatibility and v3 through R07 compatibility, always retaining the source version.
